  ...
  <div class="shoppingCart">
    ...
    <div class="cart-header">
      <div class="cart-header-content">
        <p>
          <i class="el-icon-shopping-cart-full" style="color:#ff6700; font-weight: 300;"></i>
          我的购物车
        </p>
        <span>温馨提示:没付钱是不会发货的,赶紧结算吧</span>
      </div>
    </div>
    <!-- 购物车头部END -->
    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <!-- 购物车列表 -->
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.orderId">
          <div class="pro-check">
          ...
          <div class="pro-name">
            <router-link
                :to="{ path: '/products/details', query: {productID:item.productId} }"
          ...
          <div class="pro-total pro-total-in" >{{parseFloat((item.productPrice*item.productCount).toFixed(2))}} 元</div>
          <div class="pro-action">
            <el-popover placement="right" >
              <p>确定移出购物车吗？</p>
              <div style="text-align: right; margin: 10px 0">
                <el-button size="mini" type="text" @click="cancelPopover()">取消</el-button>
                <el-button
                    type="primary"
                    size="mini"
                    @click="deleteItem(item.orderId,item.productId)"
                >确定</el-button>
              </div>
              <i class="el-icon-error" slot="reference" style="font-size: 20px;"></i>
            </el-popover>
          </div>
        </li>
        <!-- 购物车列表END -->
      ...
      <div class="cart-bar">
        <div class="cart-bar-left">
          ...
          <span class="cart-total">
            共
            <span class="cart-total-num">{{getCount}}</span> 件商品，已选择
            <span class="cart-total-num">{{getCheckCount}}</span> 件
          </span>
        </div>
        <div class="cart-bar-right">
          ...
          <router-link :to="getCheckCount > 0 ? '/confirmOrder' : ''">
            <div :class="getCheckCount > 0 ? 'btn-primary' : 'btn-primary-disabled'">去结算</div>
          </router-link>
        </div>
      </div>
      <!-- 购物车底部导航条END -->
...
export default {
  ...
  methods:{
    ...
    //修改商品数量时调用
    handleChange(count,key,productId){
      //修改商品数量时,默认勾选上
      this.updateShoppingCart({key:key,prop:"check",val:true});
      //更新数据库中订单(未支付的)商品的数量
      this.postRequest("/order/updateShoppingCart",{
        cusId:this.$store.getters.getUser.cusId,
        productId:productId,
        productCount:count
      }).then(resp =>{
        if(resp.data.status == 'success'){
          this.updateShoppingCart({key:key,prop:"productCount",val:count});
          this.$notify.success("购物车数量修改成功");
        }else {
          this.$notify.error("购物车数量修改失败");
        }
      })
      .catch(err => {
    ...
    deleteItem(orderId,productId){
      this.postRequest("/order/deleteShoppingCart",{
        cusId:this.$store.getters.getUser.cusId,
        productId:productId
      }).then(resp =>{
        if(resp.data.status =='success'){
          this.deleteShoppingCart(orderId);
          this.visible = false;
    ...
    //el-popover的v-model不生效,模拟点击其他地方来关闭
    cancelPopover(){
      document.getElementsByClassName("pro-action")[0].click();
    }
  },
  computed:{
    ...mapGetters(["getShoppingCart","getCount","getTotalPrice","getCheckCount"]),
    //计算是否全选了
    isAllCheck:{
      get(){
...
.shoppingCart .content ul .pro-count {
  float: left;
  width: 150px;
  text-align: center;
}
